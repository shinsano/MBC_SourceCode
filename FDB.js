...
$('#feedback-list').on("click", '.feedback-item', function(e) {
  var target = e.target;
  target.classList.add('feedback-label')
});
...
$('.drag-item').on("click", '.icon-feedback', function(e) {
  var delete_index = parseInt(e.currentTarget.parentNode.id.replace('img', ''));
  if (labels[delete_index - 1] !== null) {
    var label_text = Object.keys(labels[delete_index - 1])
    var label_color = $(e.currentTarget.parentNode).find('img')[0]?.style['border-color'];
  
    var current_items = []
    var fds = $('#feedback-list .feedback-item');
    for (i = 0; i < fds.length; i++) {
      current_items.push(fds[i].innerText);
    }

    for (var key_index in label_text) {
      // $('#feedback-list').append(`<li class="feedback-item py-1" style="color: ` + label_color + `;">` + label_text[key_index] + `<i class="delete-icon fa fa-trash-o float-right py-1"></i></li>`);
      if (current_items.includes(label_text[key_index]) == false)
        $('#feedback-list').append(`<li class="feedback-item py-1" style="color: ` + label_color + `;">` + label_text[key_index] + `</li>`);
    }
  }  
});
      ...
      if (images[parseInt(i)] == null && temp_images.length > 0) {
        while(temp_images.length > 0) {
          ...
          if (!images.includes(temp_images[selected_index])) {
            ...
            $('#img' + (parseInt(i) + 1)).append('<img src="' + images[parseInt(i)] + '" alt="" class="w-100 h-100" style="border: solid ' + randomColor() + ' 2px;">')
            break;
          } else {
            temp_images.splice(selected_index, 1);
            temp_labels.splice(selected_index, 1);
...
$('#nextBtn').on("click", function(e) {
  ...
  var [feedbacks, non_feedbacks] = get_feedback();
  $.ajax({
      url: "/api/next_search_new2",
      type: "POST",
      data: JSON.stringify({ labels: req_labels, word1: word1, word2: word2, log: [...logData, feedbacks.join('\n'), non_feedbacks.join('\n')], feedbacks: feedbacks, sheet: 0 }),
...
function get_feedback() {
  var fd = [];
  var fds = $('#feedback-list .feedback-label');
  for (i = 0; i < fds.length; i++) {
    if (fd.includes(fds[i].innerText) == false)
      fd.push(fds[i].innerText);
  }
  
  var nfd = [];
  var nfds = $('#feedback-list .feedback-item:not(.feedback-label)');;
  for (i = 0; i < nfds.length; i++) {
    if (nfd.includes(nfds[i].innerText) == false)
    nfd.push(nfds[i].innerText);
  }
  return [fd, nfd];
}
