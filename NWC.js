...
var option1 = [1.8, 1.4, 1.4, 1, 1, 1, 0.6, 0.6, 0.2];
var option2 = [1.25, 1, 1, 0.75, 0.75, 0.75, 0.5, 0.5, 0.24];
var weights = option1;
        ...
        function (data) {
          if (is_next) {
            ...
            if (non_empty_data != null && non_empty_data.length !== 0) {
              fill_empty_image_next(non_empty_data)
            }
          } else {
...
function fill_empty_image_next(non_empty_data) {
  // for (item in data) {
  //   data[item] = data[item].filter(n => !deleted_images.includes(n));
  // }
  var remain_count = images.filter(item => item == null).length;
  ...
  var inserted_count = 0;
  while (inserted_count != remain_count) {
    var new_image = new_image = non_empty_data[Math.round(Math.random() * (non_empty_data.length - 1))];
    if (!tmp_stack.includes(new_image)) {
      tmp_stack.push(new_image);
      queryImageLabels(new_image);
    } else {
...
$('#nextBtn').on("click", function(e) {
  ...
  //   }
  // }
  search(word1 + '%20' + word2, true);
  // $.ajax({
  //     url: "/api/next_search_new",
  //     type: "POST",
  //     data: JSON.stringify({ labels: req_labels }),
  //     contentType: "application/json",
  //     dataType: "json",
  //     cache: false,
